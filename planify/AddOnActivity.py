...
def run():
    ...
    for idx, activity in enumerate(st.session_state.add_on_activities):
        ...
        cols[0].text_input("Name", value=activity["name"], key=f"addon_name_{idx}")
        cols[1].slider("Importance", 1, 10, value=activity["importance"], key=f"addon_importance_{idx}")
        cols[2].number_input("Duration (hours)", min_value=1, value=activity["duration"], key=f"addon_duration_{idx}")
